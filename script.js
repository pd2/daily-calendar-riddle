...
const pcsList = ["sSpcs","sLpcs","bSpcs","eLpcs","lSpcs","bLpcs","Cpcs","Tpcs","Ipcs","Qpcs"];
...
function updSquaresPos(date){
  var newDate = new Date(date);
  var wdNum = newDate.getDay();
  var dNum = newDate.getDate() -1;
  var mNum = newDate.getMonth();

  const wdSqr = document.getElementById("weekdaySquare");
  const dSqr = document.getElementById("daySquare");
  const mSqr = document.getElementById("monthSquare");

  if ( ! isNaN(wdNum) && ! isNaN(dNum) && ! isNaN(mNum) ) {
    wdSqr.setAttribute('x', (wdNum * 100 ) + 100);
    wdSqr.setAttribute( 'y', 300);
    dSqr.setAttribute( 'x', ( ( dNum % 7  ) * 100 ) + 100);
    dSqr.setAttribute( 'y',( Math.trunc( dNum / 7 ) * 100 ) + 400);
...
function computePcsPos(pcs) {
  ...
  if ( ( ( x % gridStep ) == 0 ) && ( ( y % gridStep ) == 0 ) ) {
    ...
    if ( ( x>=0 && y>=0 && x<6 && y < 7 ) || ( x== 6 && y > 2 && y < 7) || ( y==7 && x>= 0 && x<3)) {
